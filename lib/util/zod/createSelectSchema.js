 ...
 * @param {object} options - Additional options including JSONB schemas
 * @returns {string} - Zod schema method call
 */
function mapPgTypeToZod(pgType, columnInfo, options = {}) {
  const { maxLength, nullable, defaultValue } = columnInfo;
  const { jsonbSchemas = {}, columnName } = options;
  ...
  switch (baseType) {
    ...
    case 'json':
    case 'jsonb':
      // Check for custom JSONB schema definition
      if (columnName && jsonbSchemas[columnName]) {
        zodSchema = jsonbSchemas[columnName];
      } else {
        // Default fallback - could be enhanced with common patterns
        zodSchema = 'z.unknown()';
      }
      break;
    ...
    case 'bigint[]':
      ...
      const elementSchema = mapPgTypeToZod(arrayBaseType, { ...columnInfo, maxLength: null }, options);
...
/**
 * Provides common JSONB schema patterns for typical use cases
 * @returns {object} - Object with common JSONB pattern generators
 */
function getCommonJsonbPatterns() {
  return {
    // Simple key-value object
    record: (valueType = 'z.unknown()') => `z.record(${valueType})`,
    
    // String-to-string mapping
    stringRecord: () => 'z.record(z.string())',
    
    // Array of objects
    objectArray: (objectSchema = 'z.unknown()') => `z.array(${objectSchema})`,
 ...
 * @param {object} options.jsonbSchemas - Custom Zod schemas for JSONB columns (optional)
 * @param {object} options.jsonbPatterns - Use common patterns for JSONB columns (optional)
 * @returns {Promise<string>} - Generated Zod schema code as string
 */
async function createSelectSchema(knex, tableName, options = {}) {
  const { columns, exclude = [], strict = true, jsonbSchemas = {}, jsonbPatterns = {} } = options;
  ...
  try {
    ...
    for (const columnName of columnsToProcess) {
      ...
      // Merge JSONB patterns into schemas for this column
      // jsonbSchemas take priority over jsonbPatterns
      const effectiveJsonbSchemas = { ...jsonbSchemas };
      if (!effectiveJsonbSchemas[columnName] && jsonbPatterns[columnName] && patterns[jsonbPatterns[columnName]]) {
        effectiveJsonbSchemas[columnName] = patterns[jsonbPatterns[columnName]]();
      }
      
      const zodType = mapPgTypeToZod(column.type, column, { 
        jsonbSchemas: effectiveJsonbSchemas, 
        columnName 
      });
